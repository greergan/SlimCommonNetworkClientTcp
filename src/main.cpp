...
Connection::Connection(std::string_view host, const uint16_t port, const bool is_tls, const int timeout_ms) {
    ...
    if(is_tls_) {
        throw network::NetworkException(network::ErrorStatus::TlsNotSupported);
    }
#endif
    init_socket(host, port, timeout_ms);
#ifdef SLIM_TLS_ENABLED
    if(is_tls_) {
        ssl_ctx_ = SSL_CTX_new(TLS_client_method());
        owns_ssl_ctx_ = true;
        init_tls(host, timeout_ms);
    }
#endif
}
...
void Connection::init_socket(std::string_view host, const uint16_t port, const int timeout_ms) {
    ...
    auto ip_info_errno = inet_pton(AF_INET, host.data(), &ip_addr);
    switch(ip_info_errno) {
        case 0: {
            ...
            auto address_info_errno = getaddrinfo(host.data(), NULL, &hints, &addrinfo_);
            if(address_info_errno == 0) {
                ...
                throw network::NetworkException(network::ErrorStatus::SocketAddressResolutionFailed,
                    std::format("{} => {} => {}", __func__, host, gai_strerror(address_info_errno)));
            }
        }
        break;
        case 1:
        ...
        default:
            throw network::NetworkException(network::ErrorStatus::SocketAddressConversionFailed,
                std::format("{} => {} => {}", __func__, host, strerror(errno)));
    }
    ...
    if(valid_host) {
        ...
        if(socket_handle_ < 0) {
            throw network::NetworkException(network::ErrorStatus::SocketCreationFailed,
                std::format("{} => {}", __func__, strerror(errno)));
        }
        ...
        if(so_error != 0) {
            throw network::NetworkException(network::ErrorStatus::SocketConnectionFailed,
                std::format("{} => {}:{} => {}", __func__, host, port, strerror(so_error)));
        }
    }
}

#ifdef SLIM_TLS_ENABLED
void Connection::init_tls(std::string_view host, const int timeout_ms) {
    ...
    if(ssl_ == nullptr) {
        throw network::NetworkException(network::ErrorStatus::TlsHandleCreationFailed,
            std::format("{} => {}", __func__, ERR_reason_error_string(ERR_get_error())));
    }
...
network::ErrorStatus Connection::read(std::vector<uint8_t>& buf, const int timeout_ms) {
    while(true) {
        ...
        if(poll_result == 0) {
            ...
            return network::ErrorStatus::ReadPollFailed;
        }
...
network::ErrorStatus Connection::write(std::string_view payload, std::vector<uint8_t>& buf, const int timeout_ms) {
    size_t total_sent = 0;
    while(total_sent < payload.size()) {
        struct pollfd pfd = { socket_handle_, POLLOUT, 0 };
        auto poll_result = poll(&pfd, 1, timeout_ms);
        if(poll_result == 0) {
            return network::ErrorStatus::WriteTimedOut;
        }
        else if(poll_result < 0) {
            return network::ErrorStatus::WritePollFailed;
        }
#ifdef SLIM_TLS_ENABLED
